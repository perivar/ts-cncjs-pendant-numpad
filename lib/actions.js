...
class Actions {
    ...
    constructor(connector, options) {
        ...
        this.connector.subscribeMessage('serialport:read', (data) => {
            if (!this.joggingAck &&
                ...
                npmlog_1.default.debug(LOGPREFIX, `Receiveed data: ${data}, setting jogginAck to true`);
                this.joggingAck = true;
            }
        });
    }
    ...
    smoothJog(x, y, z, jogSpeed, firstDelay = false) {
        npmlog_1.default.debug(LOGPREFIX, `Smooth Jog method, smoothJogging is ${this.smoothJogging}, joggingAck is ${this.joggingAck}`);
        if (!this.smoothJogging) {
            return;
        }
        ...
        this.smoothJoggingTimer = setTimeout(this.smoothJog.bind(this), SMOOTHJOG_COMMANDS_INTERVAL * jogDelayModifier - (firstDelay ? 50 : 0), x, y, z, jogSpeed);
    }
