...
exports.Actions = exports.XYZCoords = void 0;
...
class Actions {
    ...
    constructor(connector, options) {
        this.numpadState = {}; // state of current controller
    ...
    onUse(id, state) {
        ...
        const deadmanFast = b.KEYCODE_BUTTON_LTRIGGER ||
            b.KEYCODE_BUTTON_RTRIGGER ||
            a.AXIS_LTRIGGER == 1 ||
            a.AXIS_RTRIGGER == 1;
        ...
        if (id === 'KEYCODE_BUTTON_THUMBR' && b.KEYCODE_BUTTON_THUMBR) {
            ...
            this.thumbLeftActive = false;
            npmlog_1.default.debug(LOGPREFIX, `Left Thumb Enabled:${this.thumbLeftActive}, Right Thumb Enabled:${this.thumbRightActive}`);
        }
        ...
        if ((deadmanXY || deadmanZ) &&
            (id === 'AXIS_HAT_X' || id === 'AXIS_HAT_Y')) {
            this.thumbLeftActive = false;
            this.thumbRightActive = false;
            npmlog_1.default.debug(LOGPREFIX, `Left Thumb Enabled:${this.thumbLeftActive}, Right Thumb Enabled:${this.thumbRightActive}`);
        }
        ...
        // two possibly opposing numpad axes to control a
        ...
        if (deadmanXYOnly)
            condition = 'deadmanXYOnly';
        if (deadmanZOnly)
            condition = 'deadmanZOnly';
        //--------------------------------------------------
        ...
        if (id === 'AXIS_HAT_X') {
            if (a.AXIS_HAT_X === -1)
                trigger = 'AXIS_HAT_X_MINUS';
            if (a.AXIS_HAT_X === 1)
                trigger = 'AXIS_HAT_X_PLUS';
            if (condition === 'deadmanXYOnly' || condition === 'deadmanZOnly')
                trigger = null;
        }
        else if (id === 'AXIS_HAT_Y') {
            if (a.AXIS_HAT_Y === -1)
                trigger = 'AXIS_HAT_Y_MINUS';
            if (a.AXIS_HAT_Y === 1)
                trigger = 'AXIS_HAT_Y_PLUS';
            if (condition === 'deadmanXYOnly' || condition === 'deadmanZOnly')
                trigger = null;
        }
        else if (b[id]) {
    ...
    //--------------------------------------------------------------------------
    performTask(trigger, condition) {
        var _a, _b;
        const taskArray = (_b = (_a = this.options.actionsMap) === null || _a === void 0 ? void 0 : _a[trigger]) === null || _b === void 0 ? void 0 : _b[condition];
        if (taskArray) {
            const task = Array.isArray(taskArray) ? taskArray[0] : taskArray;
            const params = Array.isArray(taskArray) ? taskArray.slice(1) : null;
            npmlog_1.default.debug(LOGPREFIX, `trigger:${trigger} condition:${condition} task: ${task} params: ${params}`);
