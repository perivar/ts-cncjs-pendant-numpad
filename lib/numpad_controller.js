...
const findPath = (vendorId, productId, interfaceNumber) => {
    if (vendorId && productId) {
        npmlog_1.default.info(LOGPREFIX, `Looking for keyboard with VID:PID: ${vendorId}:${productId}`);
        ...
        if (deviceInfo) {
            npmlog_1.default.info(LOGPREFIX, `Successfully found device with VID:PID: ${vendorId}:${productId}`);
            return deviceInfo.path;
        }
        else {
            npmlog_1.default.info(LOGPREFIX, `Failed finding device with VID:PID: ${vendorId}:${productId}`);
            return undefined;
        }
    }
    else {
...
class NumpadController {
    constructor(options) {
        ...
        if (options.list) {
            npmlog_1.default.info(LOGPREFIX, `Serialports found:`);
        ...
        if (options.devicelist) {
            npmlog_1.default.info(LOGPREFIX, `USB devices found:`);
            if (options.vendorId && options.productId) {
                npmlog_1.default.info(LOGPREFIX, `Keyboard HID Address: ${findPath(options.vendorId, options.productId)}`);
            }
            else {
                const devices = node_hid_1.default.devices();
                devices.forEach(device => {
                    npmlog_1.default.info(LOGPREFIX, `Manufacturer: ${device.manufacturer}`);
                    npmlog_1.default.info(LOGPREFIX, `VendorId: ${device.vendorId}`);
                    npmlog_1.default.info(LOGPREFIX, `ProductId: ${device.productId}`);
                    npmlog_1.default.info(LOGPREFIX, `Interface: ${device.interface}`);
                    npmlog_1.default.info(LOGPREFIX, `Path: ${device.path}\n`);
                });
            }
            process.exit(1);
        }
    ...
    keyboardEventOn(data) {
        ...
        npmlog_1.default.info(LOGPREFIX, `Key: 0x${keyHex}`);
