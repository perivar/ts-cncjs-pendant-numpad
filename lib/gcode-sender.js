...
class GcodeSender {
    ...
    moveGantryRelative(x, y, z, mmPerMin) {
        ...
        if (mmPerMin) {
            ...
            this.sendMessage('command', 'gcode', `G91 G0 X${x.toFixed(4)} Y${y.toFixed(4)} Z${z.toFixed(4)}`);
        }
    ...
    performZProbingTimes2() {
        ...
        this.sendMessage('command', 'gcode', 'G91'); // relative coordinates
        this.sendMessage('command', 'gcode', `G38.2 Z-${this.probeDistance} F${this.probeFastFeedrate}`); // probe toward stock
        this.sendMessage('command', 'gcode', 'G0 Z1'); // lift up just a bit
        this.sendMessage('command', 'gcode', `G38.2 Z-2 F${this.probeSlowFeedrate}`); // probe toward stock
        this.sendMessage('command', 'gcode', 'G90'); // absolute coordinates
    ...
    //--------------------------------------------------------------------------
    sendMessage(eventName, directive, data) {
        if (!this.options.simulate && this.connector.serialConnected) {
            ...
            if (eventName == 'command') {
                if (directive == 'gcode')
                    npmlog_1.default.info(LOGPREFIX, `Gcode ${data}`);
                else
                    npmlog_1.default.info(LOGPREFIX, `Command ${directive}`);
            }
            else
                npmlog_1.default.warn(LOGPREFIX, `Unknown command ${eventName}: ${directive}, ${data}`);
        }
    }
}
exports.GcodeSender = GcodeSender;
