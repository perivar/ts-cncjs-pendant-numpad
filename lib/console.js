...
const logDevice = (hidDevice) => {
    ...
    npmlog_1.default.info(LOGPREFIX, `VendorId: ${decimalToHex(hidDevice.vendorId)} (${hidDevice.vendorId})`);
    npmlog_1.default.info(LOGPREFIX, `ProductId: ${decimalToHex(hidDevice.productId)} (${hidDevice.productId})`);
...
function startCLI() {
    ...
    if (cliOptions.devicelist) {
        ...
        if (cliOptions.vendorId && cliOptions.productId) {
            ...
            devices.forEach(hidDevice => {
                logDevice(hidDevice);
                npmlog_1.default.info(LOGPREFIX, `----------------------`);
            });
        }
        return;
    }
    ...
    if (!options.port) {
        if (options.simulate) {
            npmlog_1.default.info(LOGPREFIX, `Simulating with dummy port: dummyPort`);
            ...
            serialport_1.default
                .list()
                .then(ports => {
                npmlog_1.default.info(LOGPREFIX, `Please specify one of these using --port`);
                ports.forEach(port => {
                    npmlog_1.default.info(LOGPREFIX, `${port.path}`);
                });
            })
