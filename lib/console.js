...
//----------------------------------------------------------------------------
function startCLI() {
    const cliOptions = configureCLI(commander_1.program, version_1.version)
        .parse()
        .opts();
    if (cliOptions.list) {
...
function loadOptionsFile(filename, optionsVersion) {
    ...
    if (!fs_1.default.existsSync(filename)) {
        npmlog_1.default.info(LOGPREFIX, 'Failed! No config file at:', filename);
        return {};
    }
    else {
        npmlog_1.default.info(LOGPREFIX, 'Success! Found config file at:', filename);
    }
    try {
...
function getFileOptions(optionsVersion) {
    if (os_1.default.platform() == 'win32') {
        ...
        const dfltOpts = loadOptionsFile(path_1.default.resolve(__dirname, 'cncjs-pendant-numpad.rc.json'), optionsVersion);
