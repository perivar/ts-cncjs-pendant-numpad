...
import { NumpadController, NumpadState } from './numpad_controller';
...
export class Actions {
  ...
  numpadState = {} as NumpadState; // state of current controller
  ...
  //--------------------------------------------------------------------------
  onUse(id: string, state: NumpadState) {
    ...
    if (id === 'KEYCODE_BUTTON_THUMBR' && b.KEYCODE_BUTTON_THUMBR) {
      ...
      log.debug(
        LOGPREFIX,
        `Left Thumb Enabled:${this.thumbLeftActive}, Right Thumb Enabled:${this.thumbRightActive}`
      );
    }
    ...
    // Do the work.
    //--------------------------------------------------
    this.performTask(trigger as string, condition as string);
  } // onUse()
  ...
  performTask(trigger: string, condition: string) {
    ...
    if (taskArray) {
      ...
      log.debug(
        LOGPREFIX,
        `trigger:${trigger} condition:${condition} task: ${task} params: ${params}`
      );
